...
履歴と Markdown / JSON エクスポート。

**ウィンドウ側**: 単一カラムのトランスクリプト（通常表示 / 密表示）、日付の
区切り、テキスト検索と送信者フィルタ、最新へジャンプ、各エージェントの登録
コマンドとロールプロンプトをコピーするワンクリックのセットアップ、そして
メッセージ内のリポジトリ相対パスから実ファイルを開くリンク（`repoPath` の
内側に限定しているため、任意のパスが開かれることはありません）。
